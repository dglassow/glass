...
**Phases 0–1 done. Phase 2 M1–3 (auth, enrollment, passkey, relay/TLS/mutual-auth), Phase 3 M1–2, Phase 4 M1, Phase 5 M1 all done. No package is a skeleton.**
...
- Phase 3 M1 vault (plan §9): envelope encryption in `hub/src/vault/` over `node:sqlite` (verified flag-free on Node 25; zero native deps). A random master key wrapped in two independent scrypt keyslots (passphrase + user-supplied recovery key — LUKS-style, so recovery-only unlock works); per-secret AES-256-GCM data keys; AAD binds `vaultId+secretId+version+class` (defeats byte-flip, row-transplant, class-flip). Per-device allow-list enforced against the handshake identity (deny-by-default, before decrypt); tags are taxonomy only; two classes (workflow machine-readable, personal refused to machines with `biometric_required` — the deferred Touch ID seam). Hash-chained audit log; recovery-key entropy gate (≥90 bits). `hub vault` CLI (init/add/update/remove/reveal/list/allow/deny/tag/untag/check-recovery/audit; passphrases + values on stdin, never argv). Machine retrieval over the hub via `vault.get`→`vault.secret`, gated to authenticated (non-`--open`) mode. New `@glass/cli` package: `glass run` injects secrets into a child's env only (never argv/disk), with a structural `RedactingLogger`. `tests/p3m1-vault.mjs` (25 checks).
- Phase 3 M2 backup bundle (plan §10): `hub backup create|restore` (`hub/src/vault/backup.ts`). A single encrypted file — vault DB via `VACUUM INTO` (consistent snapshot, never a live-file copy), plus the trust + credential stores, encrypted under a scrypt key derived from the vault passphrase (separate salt, own AAD). Not in the bundle: enclave device keys, Etch. `tests/p3m2-backup.mjs` (9 checks) runs the §16 drill: wipe everything, restore on "clean hardware", secret + trust store come back and the recovery key still unlocks; wrong-passphrase and tampered bundles refuse.
- Phase 4 M1 blue/green worker swap (plan §3/§4/§12): the `supervisor` is now real (protocol-free, `supervisor/src/{proc,supervisor,control}.ts`). It spawns/monitors sessiond + the worker; a control socket (`swap <entry>`) does the blue/green swap — `standby` blue, spawn green, health-check green (it must reach sessiond AND complete the authenticated hub handshake before writing `READY` on fd 3), then retire blue only after `READY`; if green fails, blue resumes (instant rollback). The agent gains `--supervised` (fd-3 status, stdin `standby`/`resume`/`drain`). Sessions are untouched — PTY fds live in sessiond, not the swapped worker. `tests/p4m1-swap.mjs` (9 checks): health-before-retire ordering, shell never interrupted (same pid), scrollback spans the swap with non-reset seq, and a broken swap rolls back with the session intact. Fd-handoff research (red-teamed): sessiond self-update is feasible in pure Node via fd-inheritance-at-spawn (`node-pty` exposes `.fd`; no native SCM_RIGHTS needed) — that is a later milestone, along with signed-tag update distribution (§4).
- Phase 5 M1 chat provider (plan §1, §5): `sessiond` now has a provider-agnostic `Session` interface (`sessiond/src/session.ts`); `PtySession` and the new `ChatSession` both implement it, so the socket server, scrollback, and fan-out are provider-blind. `ChatSession` runs `etch -z "<message>"` per message (`sessiond/src/etch.ts`) and renders the reply into the same output stream — so a chat rides the identical session protocol and survives a worker restart like a PTY. The agent detects Etch and reports it in the device record (detected, never managed). Etch binary is overridable via `GLASS_ETCH_BIN`. `tests/p5m1-chat.mjs` (5 checks, stub etch). **Open-question Q3 resolved:** Etch's `-z`/oneshot mode gives final assistant text (not JSON) — adequate for the chat provider now; the richer `tui_gateway` JSON-RPC / ACP integrations (session ids, tool calls, cost) are a later enhancement.

Protocol note: M2 added `roles` + `deviceName` to `Hello`. P2-M1 added `hello.challenge`/`hello.proof`, enrollment fields, and `protocol/src/auth.ts`. P2-M2 added the `credential.*` family. P3-M1 added the `vault.*` family, four secret error codes, and `protocol/src/redact.ts`. All additive; `PROTOCOL_VERSION` stays 1 (unreleased). Keychain/Secure-Enclave key storage stays deferred behind the `Signer` seam (needs a Mac).
- Phase 2 M3 relay + TLS + mutual auth (plan §2/§4): decided on a **reverse tunnel** (hub dials out via `ssh -R`; the VPS runs stock sshd and only sees ciphertext) — not a Glass-aware relay. Hub gains `--tls-cert/--tls-key` (TLS terminates in the hub: `https.createServer` + `ws {server}`) and `--hub-key <file>` (its Ed25519 identity). **Mutual auth:** a spoke sends a `clientNonce`; the hub's `hello.challenge` gains a `hub{key,signature}` proof over `buildHubAuthPayload(deviceId, clientNonce, hubNonce, channelBinding)`; the spoke (`--hub-key <pin>`) verifies against its pinned key. **Channel binding** = the TLS exporter (`exportKeyingMaterial(32,"glass/cb/v1")`), so a TLS-terminating MITM relay — whose two legs export different values — is refused even with the correct pin (proven in `tests/p2m3-relay.mjs`, 6 checks, incl. a real MITM relay). `hub tunnel -- <cmd>` keeps the reverse tunnel alive (respawn). Lightsail provisioning is `infra/lightsail/` (Terraform + cloud-init) for the owner to `terraform apply` under AWS SSO — nothing billable is run from here. Browser/PWA spokes verify the pinned key over the nonces + rely on WebPKI+CAA (no exporter API); full browser MITM-immunity (X25519 AKE / E2E) is a later milestone.

Protocol note (P2-M3): `Hello` gained optional `clientNonce` + `channelBinding`; `HelloChallenge` gained an optional `hub` proof block; `auth.ts` gained `verifySignature` + `buildHubAuthPayload`/`verifyHubAuth`. All additive.
