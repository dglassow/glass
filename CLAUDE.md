...
**Phases 0–4 and 6–9 done. Phase 5 is half done: the one-shot chat provider and shared Agent Board exist; voice and the focused PWA chat surface remain. Etch is the primary structured agent, with Codex app-server, Claude streaming JSON, and configured generic CLI adapters. Glass runs against live infrastructure: a signed+notarized desktop app, a deployed relay, and hub-served signed auto-updates.**

Verification: 28 adversarial test harnesses under `tests/`, all run by `pnpm test` and CI. ARM64 macOS CI also assembles the provenance-bound backend and builds and inspects the real unsigned app. `pnpm test:providers -- --runs` is the release-machine gate for installed Etch, Codex, and Claude. Each security-sensitive milestone (updater, git hosting, reachable hub, enrollment, update banner) was additionally red-teamed via multi-agent workflows, with every confirmed bypass fixed and regression-tested. Still not machine-verified: interactive GUI behavior (a human runs the real app), the signed/notarized 0.1.7 update drill, and live sessiond fd handoff.
...
- M3 shipped-app integration: per-user LaunchAgent → stable `glassd` → Hub + supervisor. `glass-backend.mjs` is now a short-lived Tauri control client; app/window exit cannot signal the backend. Bundled releases stage under `~/.glass/runtimes/<version+digest>`; glassd blue/green-swaps Agent, restarts Hub on its stable loopback port, and keeps the exact sessiond/shell PIDs pinned. Explicit Reconfigure is the destructive stop boundary. Fresh untrusted spokes may report `SESSIOND_READY` for initial enrollment, but swaps still require fully authenticated `READY`. `tests/p4m3-persistent-service.mjs` proves app exit, Viewer replacement, Hub+Agent activation, contiguous scrollback, live I/O, and failed-green rollback end to end.
- M4 stable service update: changed `glassd` and portable Node files stage beside the installed service. Doctor shows the content-free pending state and requires explicit confirmation before the destructive LaunchAgent restart. The control client validates the replacement controller identity and configured stack, then restores and restarts the previous files if health fails. `tests/p4m4-service-update.mjs`. Backend provenance binds source commit, release version, dirty state, and runtime digest; packaging requires a clean tag at HEAD.

### Phase 5 — chat (M1 done; voice + PWA chat surface remain)

- M1 chat provider (plan §1/§5): `sessiond` has a provider-agnostic `Session` interface; `PtySession` and `ChatSession` both implement it, so the socket server, scrollback, and fan-out are provider-blind. `ChatSession` runs `etch -z "<message>"` per message and renders the reply into the same output stream; a chat rides the identical session protocol and survives a worker restart like a PTY. Etch is detected and reported in the device record (never managed); binary overridable via `GLASS_ETCH_BIN`. `tests/p5m1-chat.mjs` (stub etch). This is now the compatibility baseline, not the target integration: Phase 9 makes Etch primary through its versioned surface protocol and keeps `-z` only for older Etch versions.
...
- `@glass/desktop` is a real, signed macOS app, not a scaffold (still workspace-excluded; needs Rust on a Mac). `pnpm tauri build` produces Glass.app / Glass.dmg with the backend **bundled**: `@glass/backend-bundle` pnpm-deploys hub/sessiond/agent/supervisor (+ node-pty darwin-arm64 prebuild) into a flat tree, plus `glassd` and a portable official node binary, into app Resources; the .dmg runs on a Mac with no node and no repo. `deploy/glass-backend.mjs` is a short-lived role/control client; it installs/attaches to persistent glassd and returns one `GLASS_BACKEND_READY` line. First-run role picker (Standalone/Hub/Spoke) in `viewer/src/onboarding.ts`; native menus (Edit for clipboard, Reconfigure, Terminal Settings ⌘,); tiling session workspace (`workspace.ts`: binary layout tree, drag-to-split, hide≠kill); customizable terminal appearance over a transparent window (`macos-private-api`). Signing: `sign-and-notarize.sh` deep-signs inside-out (native addons → bundled node with JIT entitlements → app), notarizes app+dmg via the `glass-notary` notarytool Keychain profile, staples both. Mac App Store is a non-starter (App Sandbox forbids spawning node/shells); this uses Developer ID direct distribution deliberately.
...
### Phase 9: Etch-primary multi-agent control plane (complete)

- `sessiond` owns normalized runs and provider processes. Etch uses public `etch surface --stdio` lifecycle/stream/input/delegation/orchestration; older Etch falls back visibly to reduced `etch -z`. Etch owns its isolated worktrees and all transcript, context, policy, delegation, and autonomy state.
- Codex uses its persistent public app-server thread protocol with streaming, input/approval routing, usage, interrupt, and resume, with `codex exec --json` fallback. Claude uses the resumable streaming-JSON CLI. Generic commands require owner-configured `GLASS_GENERIC_AGENT_ARGV` and expose bounded stdio/interrupt only.
- Hub persists 0600 run/workspace metadata only; content stays provider-native and point-to-point to subscribed viewers. Agent subscriptions are reconstructible. Viewer/Agent replacement leaves runs alive in sessiond. Every Agent registration forwards a UUID-tagged sessiond inventory so stale nonterminal cards become interrupted after a daemon restart.
- The shared Viewer has an Agent Board with Needs you/Active/Finished lanes, launch profiles, Etch/isolated-worktree defaults, stream/input/control/delegation UI, inspectors, attachments, durable workspace selection, provider-native resume, and content-free Doctor diagnostics.
- `tests/p9m1-agent-runs.mjs` and `tests/p9m2-mixed-providers.mjs` prove Etch negotiation/delegation/status, concurrent Codex+Claude, injected failure isolation, approval isolation, subscriber-only content, metadata-store privacy, usage, workspaces, in-flight Agent replacement, daemon-restart reconciliation, and provider resume. `tests/provider-live-smoke.mjs` exercises the actual installed provider trio before release.

### Protocol notes

`PROTOCOL_VERSION` stays 1 (unreleased); every change so far is additive. Families: `handshake` (mutual auth and provider detection), `device`, `session` (PTY/chat lifecycle and streams), `run.*` (normalized provider lifecycle, streams, input/control, queries, attachments), `workspace.*` (Agent Board metadata), `credential.*`, `vault.*`, `proxy.*`, and `system`. Provider-native protocols remain private to sessiond adapters. `protocol/src/auth.ts` holds the isomorphic signing/verification helpers; `protocol/src/redact.ts` the secret-redaction structure.
...
**Etch is detected and invoked, never managed.** It's a separate CLI the owner installs and configures by hand. Report presence, version, and advertised capabilities; never write install, update, configuration, or bundling logic for it. The structured adapter targets Etch's versioned surface contract and must not depend on private gateway methods.

**Glass orchestrates agents; providers own their runtime state.** A durable agent run is control metadata layered over a `pty` or `chat` session, not a new session kind. Etch remains authoritative for Lattice/Silica context, transcripts, model/tool schemas, permissions, skills, delegation, autonomy, and worktrees. Glass stores only bounded routing, capability, status, attention, and workspace metadata.
